...
ParticleEngine.initParticle = function ( i, positions, velocities, colors, sizes, lifetimes, fireworkStyles, materials) {
    // positions
    positions[ 3 * i + 0 ] = 0;
    positions[ 3 * i + 1 ] = 50;
    positions[ 3 * i + 2 ] = 0;
    ...
    materials[i] = SystemSettings.fireMaterial;
...
ParticleEngine.cubeEmitter = function ( count ) {
	...
	var fireworkStyles = new Float32Array(count);
    var materials = new Float32Array(count);

    for( var i = 0; i < count; i++ ) {
        console.log("BREAKPOINT1");
        this.initParticle( i, positions, velocities, colors, sizes, lifetimes, fireworkStyles, materials);
    }
	...
	particles.addAttribute('fireworkStyle', new THREE.BufferAttribute(fireworkStyles, 1));
    particles.addAttribute('material', new THREE.BufferAttribute(materials, 1));
};
...
ParticleEngine.update = function () {
	...
	var fireworkStyles = this._particles.getAttribute('fireworkStyle').array;
    var materials = this._particles.getAttribute('material').array;
	...
	this._particles.attributes.fireworkStyle.needsUpdate = true;
    this._particles.attributes.material.needsUpdate = true;
}
